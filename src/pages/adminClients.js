...
function adminClients(props) {
    const [client, setClient] = useState({ title: '', image: null })
    ...
    const getClients = async () => {
        const { response, error } = await httpRequest("GET", "/client/getAll",null,{"Authorization":props.login.token});
        if (!error) {
    ...
    const onFileUpload = async () => {
        ...
        axios.post(`${process.env.BACKEND_URL}/client`, formData,{headers: {"Authorization":props.login.token}}).then(data => {
            cogoToast.success(data.data.message)
            getClients();
        })
    };
    const handleDelete=async(id)=>{
        const { response, error } = await httpRequest("DELETE", `/client/${id}`,null,{"Authorization":props.login.token});
        if (!error) {
    ...
    return (
        <PageAuthWrapper>
            <div className="text-center text-2xl p-4 underline">
                <p>Add and Manage your clients or Paterners here</p>
            </div>
                    ...
                    <form className="w-full max-w-sm border-2 border-grey-500 p-4">
                        <div className="md:flex md:items-center mb-6">
                            ...
                            <div className="md:w-2/3">
                                <input className="bg-white appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500" id="inline-full-name" type="text" value={client.title} name="title" onInput={handleInput} />
                            </div>
                        </div>
                        <div className="md:flex md:items-center mb-6">
                            <div className="md:w-1/3">
