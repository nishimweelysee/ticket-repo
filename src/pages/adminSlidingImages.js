...
function adminSlidingImages(props) {
    ...
    const getClients = async () => {
        const { response, error } = await httpRequest("GET", "/image/getAll/slide",null,{"Authorization":props.login.token});
        if (!error) {
    ...
    return (
        <PageAuthWrapper>
            <div className="text-center text-2xl p-4 underline dark">
                <p>Add and Manages images for your publication on main page (landing page)</p>
            </div>
